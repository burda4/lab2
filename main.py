...
class MyApp(server.App):
    ...
    def getData(self, params):
        ...
        data = pd.read_csv('clean_vhi_id_{} 2017 04 04.csv'.format(city), index_col=False, header=1, skipfooter=1,
                         engine='python',
                         names=['year', 'week', 'SMN', 'SMT', 'VCI', 'TCI', 'VHI'], delimiter='\,\s+|\s+|\,')
        df = pd.DataFrame(data[data.year == int(params['year'])][data.week >= int(params['week1'])][data.week <= int(params['week2'])][
                               ["year", "week", params['index']]])
        return df
